    ...
    : QSyntaxHighlighter(parent)
{
    ...
    keywordFormat.setForeground(QColor(205,133,0)); //关键字
...
//    classFormat.setFontWeight(QFont::Bold);   //Qt类名
//    classFormat.setForeground(Qt::darkMagenta);
//    rule.pattern = QRegularExpression(QStringLiteral("\\bQ[A-Za-z]+\\b"));
//    rule.format = classFormat;
//    highlightingRules.append(rule);

    quotationFormat.setForeground(QColor(50,200,250));  //字符串
    rule.pattern = QRegularExpression(QStringLiteral("\".*\""));
    rule.format = quotationFormat;
    highlightingRules.append(rule);

    //functionFormat.setFontItalic(true);     //函数名
    functionFormat.setForeground(QColor(250,200,0));
    rule.pattern = QRegularExpression(QStringLiteral("\\b[A-Za-z0-9_]+(?=\\()"));
    rule.format = functionFormat;
    highlightingRules.append(rule);

    singleLineCommentFormat.setForeground(QColor(100,200,200));   //单行注释
    rule.pattern = QRegularExpression(QStringLiteral("//[^\n]*"));
